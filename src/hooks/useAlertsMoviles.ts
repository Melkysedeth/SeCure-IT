...
interface UseAlertsMovilesOptions {
    page?: number;
    pageSize?: number;
    activoId?: string;
...
export function useAlertsMoviles(options: UseAlertsMovilesOptions = {}) {
    const { page = 1, pageSize, activoId, filters } = options;

    // Los móviles no tienen ciudad fija: si el filtro de ciudad está activo,
    // no hay match posible, así que evitamos la consulta.
    const ciudadBloqueaResultados = !!filters && filters.ciudad !== "Todas";

    const key = `alertas_moviles_con_activo:${activoId ?? "all"}:${page}:${pageSize ?? "no-limit"}:${filters?.estado ?? "Todos"}:${filters?.severidad ?? "Todas"}:${filters?.tipo ?? "Todos"}:${filters?.ciudad ?? "Todas"}:${filters?.search ?? ""}`;

    const fetcher = useMemo(() => {
        return async () => {
            if (ciudadBloqueaResultados) {
                return { data: { rows: [] as AlertaRaw[], total: 0 }, error: null };
            }

            let query = supabase.from("alertas_moviles_con_activo").select("*", { count: "exact" }).order("created_at", { ascending: false });
    ...
    }, [activoId, page, pageSize, filters, ciudadBloqueaResultados]);
